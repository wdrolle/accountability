    ...
    if (!membership) {
      ...
      if (godV2UserId) {
        ...
        if (sessionMembership) {
          ...
          const whiteboards = await prisma.agents_whiteboard.findMany({
            where: {
              group_id: groupId,
            },
            orderBy: {
...
export async function POST(
  ...
  try {
    ...
    await prisma.agents_whiteboard.deleteMany({
      where: {
        group_id: groupId,
      },
    });
    ...
    const createdWhiteboards = await Promise.all(
      whiteboards.map((board: WhiteBoardContent) =>
        prisma.agents_whiteboard.create({
          data: {
            group_id: groupId,
            created_by: godV2UserId,
            title: board.title,
