...
const SingleTeam = ({ team }: { team: Team }) => {
  ...
  return (
    <div 
      className="wow fadeInUp group text-center relative rounded-xl bg-white/10 dark:bg-dark/40 light:bg-white p-8 border dark:border-white/[0.1] light:border-black shadow-lg transform hover:-translate-y-1 hover:shadow-xl transition-all"
      style={{
    ...
    >
      <div className="relative z-10">
        <div className="team-img-gradient group-hover:before:gradient-3 relative mx-auto mb-7.5 h-50 w-full max-w-50 rounded-full transform hover:scale-105 transition-transform">
          <Image
            src={image}
            alt={name}
            fill
        ...
        <h3 className="mb-2.5 text-heading-6 font-semibold text-white dark:text-white light:text-black transform hover:-translate-y-0.5 transition-transform">{name}</h3>
        <p className="mb-6 font-medium text-gray-400 dark:text-gray-400 light:text-black/80 transform hover:-translate-y-0.5 transition-transform">{designation}</p>

        <div className="flex items-center justify-center gap-5">
          <a
            aria-label="social icon"
            href={fbLink}
            className="duration-300 ease-in hover:text-white dark:hover:text-white light:hover:text-black"
          >
            <svg
